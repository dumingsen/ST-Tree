...
from dataloader.read_UEA import load_UEA, load_UEA_exp
import time
# UEA datasets
import numpy as np
...
from model_NTree.build_exp import build_model
...
parser.add_argument('--batch_size', type=int, default=1)  # 设置batch_size的目的让模型在训练过程中每次选择批量的数据来进行处理
...
def GetDataAndNet(archive_path, archive_name, wa, prob, mask=1):
    train_x, train_y, test_x, test_y, train_loader, test_loader, num_class = load_UEA_exp(archive, args)  ##trainloader,
...
def train(optimizer):
    ...
    for epoch in range(n_epochs):
        ...
        for batch_id, (x, y) in enumerate(test_loader):
            # python run_UEA_explain.py
            print(x.shape)  # ([1, 206, 3])
            aa = 2
            if aa == 1:
                # 原始数据可视化
                if not os.path.exists(os.path.join('origintime', str(batch_id))):
                    os.makedirs(os.path.join('origintime', str(batch_id)))

                printt('========================================第{0}组数据开始可视化'.format(batch_id))
                x1 = x.reshape(x.shape[2], x.shape[1])
                for a in range(x1.shape[0]):
                    plt.plot(x1[a])
                    plt.savefig("D:/桌面/NTree-master/origintime/{0}/维度{1}.png".format(batch_id, a))
                    # plt.show()
                    plt.close()
                input()
            else:
                ...
                printt(
                    '=================================================================================================第{0}组数据开始可视化'.format(
                        batch_id))
                net.explain(x, y, f'{batch_id}-{train_y[y[0]]}')
                printt(
                    '====================================================================================================第{0}组数据结束可视化'.format(
                        batch_id))
                input()
...
if __name__ == '__main__':
    ...
    #
    # PEMS-SF
    archive = 'BasicMotions'  ##数据集
    # Epilepsy 1
    # LabelData1 BasicMotions 1
    # CharacterTrajectories 1
    # HandMovementDirection 1
    # AtrialFibrillation
    ...
    file = r'./result/result_{0}.csv'.format(archive)
    train_x, train_y, test_x, test_y, train_loader, test_loader, net, num_class = GetDataAndNet(0, archive, wa,
                                                                                                prob)  ##获取数据，网络
