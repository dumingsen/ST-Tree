...
class WindowAttention(nn.Module):  ##
    def __init__(self, t, dim, heads, head_dim, shifted, window_size, relative_pos_embedding, wa_dim, wa, prob, mask,
                 ...
                 in_channels=1, downscaling_factor=1
        ...
        if dim == 96:
            self.cbam = cbamblock(768, 16, 7)  # 768
        if dim == 192:
            self.cbam = cbamblock(384, 16, 7)  # 768
        if dim == 60:
            self.cbam = cbamblock(192, 16, 7)  # 768

        out_channels = dim
        self.downscaling_factor = downscaling_factor
        self.linear_back1 = nn.Linear(out_channels, in_channels * downscaling_factor)
        self.linear_back2 = nn.Linear(dim, in_channels * downscaling_factor)
    ...
    def forward(self, x):
        ...
        if not self.prob:  ###不稀疏
            ...
            if self.relative_pos_embedding:  ##相对位置bias
                ...
                print('self.pos_embedding[self.relative_indices[:, :].to(torch.long)]',
                      self.pos_embedding[self.relative_indices[:, :].to(torch.long)].shape)  # [32, 32])
        ...
        if self.shifted:  ##shifted
            ...
            # outback = self.linear_back1(out)
            # print('outback1',outback.shape)
            #
            # # 还原原本输入
            # outback = rearrange(outback, 'b f (p c)-> b (p f) c ',
            #                     p=self.downscaling_factor)  # p c 是一个特征   f是代表分为了多少段 说反了
            # print('outback3', outback.shape)  # ([16, 3072, 6])
            #
            # out=outback
        # else:
        # outback = self.linear_back2(out)#[16, 768, 24]
        # print('outback2', outback.shape)

        # #还原原本输入
        # outback = rearrange(outback, 'b f (p c)-> b (p f) c ', p=self.downscaling_factor)  # p c 是一个特征   f是代表分为了多少段 说反了
        # print('outback3', outback.shape)#([16, 3072, 6])
        # print('attention', out.shape)  # [2, 768, 206])
        return out  # 作为图卷积的输入
    ...
    def attn(self, query: torch.Tensor, key, value: torch.Tensor, new_p, activation="Softmax"):
        ...
        if self.relative_pos_embedding:  ##相对位置bias
            ...
            print('self.pos_embedding[self.relative_indices[:, :].to(torch.long)]',
                  self.pos_embedding[self.relative_indices[:, :].to(torch.long)].shape)  # 64, 64]
        ...
        if self.scale1:
            pre_att = pre_att / math.sqrt(value.size(-1))
        print('pre_att = pre_att / math.sqrt(value.size(-1))',
              (pre_att / math.sqrt(value.size(-1))).shape)  # torch.Size([16, 3, 768, 768])#[16, 3, 12, 64, 64])
...
class SwinBlock(nn.Module):
    def __init__(self, t, dim, heads, head_dim, mlp_dim, shifted,
        ...
        self.attention_block = Residual(PreNorm(dim, WindowAttention(t,
                                                                     ...
                                                                     wa_dim=wa_dim,
                                                                     # wa_dim=down_dim // reduce(lambda x, y: x * y,downscaling_factors[:i + 1]),#(4,2, 2,2),  # 代表多长的时间作为一个特征
                                                                     wa=wa,
                                                                     prob=prob,
                                                                     mask=mask,
                                                                     win_len=768, scale1=False, q_len=1, sub_len=6,
                                                                     sparse=True, attn_pdrop=0.1, resid_pdrop=0.1,
    ...
    def forward(self, x):
        ...
        print('attention x', x.shape)  # [16, 768, 1197]
        x = self.mlp_block(x)  #
        print('mlp x', x.shape)  # [16, 768, 1197]
        out = x
        ##可以去掉还原原本输入 用于可解释性
        # if self.shifted:
        #     outback = self.linear_back1(out)
        ...
        for _ in range(layers // 2):
            self.layers.append(nn.ModuleList([
                ...
                SwinBlock(t, dim=hidden_dimension, heads=num_heads, head_dim=head_dim, mlp_dim=hidden_dimension * 4,
                          ...
                          , in_channels=in_channels, downscaling_factor=downscaling_factor
                          ),
                ##shifted True
                SwinBlock(t, dim=hidden_dimension, heads=num_heads, head_dim=head_dim, mlp_dim=hidden_dimension * 4,
...
class attentionNet(nn.Module):
    def __init__(self, hidden_dim, layers, heads, channels, wa, prob, mask,
                 t=300, down_dim=3072, num_classes=1000, head_dim=32, window_size=7,
        ...
        for i in range(1):  # hidden_dim=(96, 192, 62),#len(hidden_dim) 1 #注意，实际实验的代码和可解释的代码不同之处
            layer = StageModule(  #
                t,
                in_channels=channels if i == 0 else hidden_dim[i - 1],
                hidden_dimension=hidden_dim[i],  # (96, 192, 62),#hidden_dim[i],叠加多层，最后一层必须是时间序列长度 t
    ...
    def forward(self, ts):  ##ts为流入的训练数据
        ...
        ds = self.downsample(ts.to(torch.float32))  # B,C,L',（ts长度，3072）#
        ...
        for Encoder in self.EncoderList:  ##
            x = Encoder(x)
            print('================================transforemr', x.shape)  # [1, 768, 206]
        encoder = x
        print('encoder', encoder.shape)  # ([16, 192, 62])#[16, 768, 62] #[4, 768, 96])
        # input()
        return ds, encoder
